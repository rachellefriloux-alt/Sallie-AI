...
class PluginRegistry {
  ...
  /**
   * Get plugins suitable for build-time injection
   */
  getBuildTimePlugins(): Plugin[] {
    return this.getAllPlugins().filter(plugin => 
      plugin.enabled && 
      plugin.health === 'healthy' &&
      (plugin.category === 'utility' || plugin.category === 'ai' || plugin.category === 'integration')
    );
  }

  /**
   * Create a build configuration for specific plugins
   */
  createBuildConfiguration(pluginIds: string[]): Record<string, any> {
    const config: Record<string, any> = {};
    
    pluginIds.forEach(id => {
      const plugin = this.plugins.get(id);
      if (plugin && plugin.enabled) {
        config[`sallie.plugin.${id}.enabled`] = true;
        config[`sallie.plugin.${id}.version`] = plugin.version;
        config[`sallie.plugin.${id}.category`] = plugin.category;
        
        if (plugin.config) {
          Object.entries(plugin.config).forEach(([key, value]) => {
            config[`sallie.plugin.${id}.config.${key}`] = value;
          });
        }
      }
    });
  ...
  /**
   * Generate Gradle properties for plugin injection
   */
  generateGradleProperties(): string {
    const buildTimePlugins = this.getBuildTimePlugins();
    const lines: string[] = [
      '# Sallie Plugin Configuration for Gradle Builds',
      '# Generated by PluginRegistry - Got it, love.',
      ''
    ];
    
    buildTimePlugins.forEach(plugin => {
      lines.push(`# Plugin: ${plugin.name} v${plugin.version}`);
      lines.push(`sallie.plugin.${plugin.id}.enabled=true`);
      lines.push(`sallie.plugin.${plugin.id}.version=${plugin.version}`);
      lines.push(`sallie.plugin.${plugin.id}.category=${plugin.category}`);
      
      if (plugin.config) {
        Object.entries(plugin.config).forEach(([key, value]) => {
          lines.push(`sallie.plugin.${plugin.id}.config.${key}=${value}`);
        });
      }
  ...
  /**
   * Subscribe to plugin changes for build system integration
   */
  onBuildSystemChange(callback: (buildConfig: Record<string, any>) => void): void {
    this.onPluginChange((event: any) => {
      if (event.plugin.category === 'utility' || event.plugin.category === 'ai' || event.plugin.category === 'integration') {
        const buildConfig = this.createBuildConfiguration(this.getBuildTimePlugins().map(p => p.id));
