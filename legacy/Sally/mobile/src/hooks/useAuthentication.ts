...
const getAuthApiBase = () => {
  ...
  return backend ? `${backend.replace(/\/$/, '')}/api/auth` : '';
};
...
export function useAuthentication() {
  ...
  const login = async (credentials: LoginCredentials): Promise<boolean> => {
    if (!API_BASE_URL) {
      setAuthState(prev => ({ ...prev, isLoading: false, error: 'Optional backend not configured. Use Supabase sign-in.' }));
      return false;
    }
  ...
  const register = async (data: RegisterData): Promise<boolean> => {
    ...
    try {
      ...
      if (!API_BASE_URL) {
        setAuthState(prev => ({ ...prev, isLoading: false, error: 'Optional backend not configured.' }));
        return false;
      }
