...
const getRealtimeWsUrl = () => {
  ...
  return backend.startsWith('https') ? `wss://${base}/ws` : `ws://${base}/ws`;
};

export function useRealtimeUpdates(config: Partial<RealtimeConfig> = {}) {
  const defaultConfig: RealtimeConfig = {
    url: getRealtimeWsUrl() || 'ws://192.168.1.47:8742/ws',
  ...
  const connect = useCallback(() => {
    if (wsRef.current?.readyState === 1) { // WebSocket.OPEN
      return;
    }
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        // Web WebSocket
        ws = new (global as any).WebSocket(finalConfig.url);
      } else {
        // React Native WebSocket
        // In a real app, you'd use a library like react-native-websockets or socket.io
        // For now, we'll simulate the connection
        console.log('Simulating WebSocket connection for React Native');
        ws = {
          readyState: 1, // OPEN
          send: (data: string) => {
            ...
            setTimeout(() => {
              ...
              if (message.type === 'ping') {
                ...
                ws.onmessage?.({ data: JSON.stringify({ type: 'pong', timestamp: Date.now() }) });
              }
            }, 100);
          },
          close: (code?: number, reason?: string) => {
            console.log('Simulated WebSocket close:', code, reason);
            ws.readyState = 3; // CLOSED
            ws.onclose?.({ code: code || 1000, reason: reason || 'Normal closure' });
          },
          onopen: null,
          onmessage: null,
          onclose: null,
