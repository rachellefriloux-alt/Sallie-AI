...
const DEFAULT_CONFIG: AnalyticsConfig = {
  ...
  batchSize: 10,
  flushInterval: 5000,
  apiEndpoint: (typeof process !== 'undefined' && process.env?.EXPO_PUBLIC_API_URL?.trim?.())
    ? `${process.env.EXPO_PUBLIC_API_URL.replace(/\/$/, '')}/api/analytics/events`
    : (typeof process !== 'undefined' && process.env?.EXPO_PUBLIC_BACKEND_URL?.trim?.())
      ? `${process.env.EXPO_PUBLIC_BACKEND_URL.replace(/\/$/, '')}/api/analytics`
      : '',
