...
export function AlignmentPage({ navigation }: any) {
  ...
  const runAdvancedPatternAnalysis = () => {
    const insights: string[] = [];
    if (alignmentPatterns.length > 0) {
      const top = alignmentPatterns.reduce((a, b) => a.confidence > b.confidence ? a : b);
      insights.push(`Strongest pattern: ${top.name} (${Math.round(top.confidence * 100)}% confidence)`);
    }
    const avgScore = alignmentMoments.length > 0
      ? alignmentMoments.reduce((s, m) => s + m.alignment_score, 0) / alignmentMoments.length
      : 0;
    insights.push(`Average alignment score: ${Math.round(avgScore * 100)}%`);
    const types = alignmentMoments.reduce((acc: Record<string, number>, m) => {
      acc[m.alignment_type] = (acc[m.alignment_type] || 0) + 1;
      return acc;
    }, {});
    const dominantType = Object.entries(types).sort(([, a], [, b]) => b - a)[0];
    if (dominantType) {
      insights.push(`Most common alignment: ${getAlignmentTypeLabel(dominantType[0])}`);
    }
    alert('Advanced Pattern Analysis\n\n' + insights.join('\n\n'));
  };
  ...
  const renderAlignmentPatterns = () => (
    <Card style={styles.card}>
      ...
      <Button 
        mode="outlined" 
        onPress={runAdvancedPatternAnalysis}
