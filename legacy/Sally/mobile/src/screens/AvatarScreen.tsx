...
export function AvatarScreen({ navigation }: any) {
  ...
  const loadSavedAvatar = async () => {
    try {
      ...
      const { data } = await supabase
        ...
        .eq('id', user.id)
        .single();
      if (data?.avatar_state && typeof data.avatar_state === 'object') {
        const saved = data.avatar_state as AvatarManifestation;
        if (saved.name) {
          setCurrentAvatar(prev => prev ?? saved);
          setManifestationHistory(prev => {
      ...
      // Ignore - avatar_state column may not exist yet
    }
  };

  const loadAvailableForms = async () => {
    try {
      const response = await fetch(`${getBackendUrl()}/avatar/infinite/forms`);
      if (response.ok) {
  ...
  const loadCurrentAvatar = async () => {
    try {
      const response = await fetch(`${getBackendUrl()}/avatar/infinite/current`);
      if (response.ok) {
        const data = await response.json();
        if (data.avatar) {
  ...
  const manifestAvatar = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${getBackendUrl()}/avatar/infinite/manifest`, {
        method: 'POST',
        headers: {
  ...
  const saveAvatar = async () => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        Alert.alert('Error', 'Please sign in to save your avatar');
        return;
      }
      const { error } = await supabase
        .from('profiles')
        .update({ avatar_state: currentAvatar })
        .eq('id', user.id);
  ...
  const evolveAvatar = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${getBackendUrl()}/avatar/infinite/evolve`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
