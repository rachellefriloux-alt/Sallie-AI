...
export function UserThoughtPage({ navigation }: any) {
  ...
  const runAdvancedPatternAnalysis = () => {
    const allThoughts = [...userThoughts];
    const sentimentWords: Record<string, number> = {};
    const timePatterns: Record<string, number> = {};
    allThoughts.forEach((t) => {
      const words = t.content.toLowerCase().split(/\s+/);
      words.forEach((w) => {
        if (w.length > 4) sentimentWords[w] = (sentimentWords[w] || 0) + 1;
      });
      const hour = new Date(t.timestamp).getHours();
      const period = hour < 12 ? 'morning' : hour < 18 ? 'afternoon' : 'evening';
      timePatterns[period] = (timePatterns[period] || 0) + 1;
    });
    const topWords = Object.entries(sentimentWords)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 5)
      .map(([w]) => w);
    const topPeriod = Object.entries(timePatterns).sort(([, a], [, b]) => b - a)[0];
    Alert.alert(
      'Pattern Analysis',
      `Most frequent themes: ${topWords.join(', ') || 'None yet'}\n\nYou think most in the ${topPeriod?.[0] ?? 'unknown'}.\n\n${userPatterns.length} recurring patterns detected.`
    );
  };
    ...
    <Card style={styles.card}>
      ...
      <View style={styles.intensityRow}>
        {[0, 0.2, 0.4, 0.6, 0.8, 1].map((v) => (
          <TouchableOpacity
            key={v}
            style={[styles.intensityBtn, Math.abs(intensity - v) < 0.15 && styles.intensityBtnActive]}
            onPress={() => setIntensity(v)}
          >
            <Text style={[styles.intensityBtnText, Math.abs(intensity - v) < 0.15 && { color: '#fff' }]}>{v}</Text>
          </TouchableOpacity>
        ))}
      </View>
