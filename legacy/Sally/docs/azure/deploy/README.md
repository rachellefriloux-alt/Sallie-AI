...
Production deployment for the Sallie Studio ecosystem on Azure.

---

## This repo: Next.js at repo root

The **web app is the Next.js project at the repository root** (not in a `web/` subfolder). It has **API routes** (chat, user, health, etc.), so you need a **Node server**:

- **Azure App Service (Linux, Node 20 LTS)** — Deploy the repo; build with `npm run build`, start with `npm run start`. Set env vars in App Service → Configuration.
- **Docker** — Use the **Dockerfile** at repo root. Build: `docker build -t sallie-web .`. Deploy the image to **Azure Container Apps** or **App Service (Linux container)**.
- **Static Web Apps** — Only supports a static export. This app uses API routes; use App Service or Docker for the full app.

The `deploy-web.sh` script in `azure/deploy/` builds from the repo root and deploys to Static Web Apps; for API routes you must use App Service or a container instead (see below). See also [docs/THIS_REPO.md](../../THIS_REPO.md) for how this repo is structured and run.

---

## Architecture overview

### **Web application** (Next.js 15) — *this repo*
- **Platform**: Azure App Service (Node) or Azure Container Apps (Docker)
- **Runtime**: Node.js 20 LTS
- **Features**: SSR, API routes, Supabase auth, Prisma + Postgres
- **URL**: Your App Service or Container Apps URL

Sections below that mention a separate Python backend, WinUI desktop, or `web/` refer to optional or alternate setups. For **this** repo, deploying the Next.js app (above) plus Supabase is sufficient.
...
### 2. **Deploy Web Application** (Next.js at repo root)

**Option A — App Service (Node)**

```bash
# From repo root (where package.json and next.config.mjs live)
cd /path/to/Sally

npm install
npm run build

# Create web app if not exists (Linux, Node 20)
az webapp create \
  --name sallie-studio-web \
  --resource-group SallieStudioRG \
  --plan SallieStudioPlan \
...
# Deploy (e.g. via zip or GitHub Actions). Startup: npm run start
...
```bash
# From repo root
docker build -t sallie-web .
# Push to ACR, then deploy to Container Apps or App Service (Linux container)
```
