...
const copyMindFeatures: { name: string; status: string; description: string; effectiveness: number; usage: number }[] = [
  { name: 'Pattern Recognition', status: 'active', description: 'Identifies recurring themes in your communication style', effectiveness: 92, usage: 156 },
  { name: 'Voice Synthesis', status: 'active', description: 'Captures your unique tone and vocabulary preferences', effectiveness: 87, usage: 89 },
  { name: 'Memory Integration', status: 'active', description: 'Links historical context to current conversations', effectiveness: 95, usage: 243 },
  { name: 'Predictive Drafting', status: 'beta', description: 'Pre-generates responses based on behavioral patterns', effectiveness: 74, usage: 45 },
  { name: 'Emotional Calibration', status: 'active', description: 'Adjusts output based on detected emotional state', effectiveness: 91, usage: 178 },
  { name: 'Context Switching', status: 'beta', description: 'Adapts between professional and personal modes', effectiveness: 68, usage: 34 },
];

export function ResearchUniverseDimension({ userState, sallieState }: ResearchUniverseDimensionProps) {
  const [activeMode, setActiveMode] = useState<'mindmap' | 'research' | 'knowledge' | 'protocols'>('mindmap');
                ...
                {copyMindFeatures.map((feature, index) => (
                  <div key={index} className="p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
                    <div className="flex items-center justify-between mb-2">
                      <h5 className="font-medium text-gray-500 text-sm">{feature.name}</h5>
          ...
          <div className="space-y-6">
            {researchProjects.map((project) => (
              <div key={project.id} className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-peacock-200">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-xl font-semibold text-gray-500">{project.title}</h3>
                ...
                {/* Findings */}
                <div className="mb-4">
                  <h4 className="font-medium text-gray-500 mb-2">Key Findings</h4>
                ...
                {/* Next Steps */}
                <div className="mb-4">
                  <h4 className="font-medium text-gray-500 mb-2">Next Steps</h4>
                ...
                {/* Resources */}
                <div>
                  <h4 className="font-medium text-gray-500 mb-2">Resources</h4>
                  <div className="flex flex-wrap gap-2">
                    {project.resources.map((resource, index) => (
                      <span key={index} className="px-2 py-1 bg-gray-100 text-gray-500 rounded-full text-xs">
                        {resource}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {tier.domains.map((domain, domainIndex) => (
                    <div key={domainIndex} className="p-4 bg-gradient-to-br from-purple-50 to-indigo-50 rounded-lg border border-purple-200">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-semibold text-gray-500 text-sm">{domain.domain}</h4>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {omniProtocols.map((protocol, index) => (
                <div key={index} className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-gold-200">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-semibold text-gray-500">{protocol.name}</h3>
                      <p className="text-sm text-gray-400">Tier {protocol.tier} Protocol</p>
                    </div>
                    <div className="text-right">
                  ...
                  {/* Process */}
                  <div className="mb-4">
                    <h4 className="font-medium text-gray-500 mb-2">Process</h4>
                    <div className="p-2 bg-gold-50 rounded-lg border border-gold-200">
                      <p className="text-sm text-gray-500">{protocol.process}</p>
                    </div>
                  </div>

                  {/* Example */}
                  <div>
                    <h4 className="font-medium text-gray-500 mb-2">Example Output</h4>
                    <div className="p-3 bg-gradient-to-r from-amber-50 to-yellow-50 rounded-lg border border-amber-200">
                      <p className="text-sm text-gray-500 italic">{protocol.example}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-gradient-to-r from-purple-100 to-indigo-100 rounded-xl p-6 border border-purple-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center p-4 bg-white rounded-lg border border-purple-200">
                  ...
                  <p className="text-sm text-gray-400 mt-2">Reverse-engineer and modify reality patterns</p>
                </div>
                <div className="text-center p-4 bg-white rounded-lg border border-purple-200">
                  <div className="text-2xl font-bold text-indigo-700">Predictive Modeling</div>
                  <p className="text-sm text-gray-400 mt-2">Forecast civilizational cycles and outcomes</p>
                </div>
                <div className="text-center p-4 bg-white rounded-lg border border-purple-200">
                  <div className="text-2xl font-bold text-gold-700">Human Optimization</div>
                  <p className="text-sm text-gray-400 mt-2">Total enhancement of human potential</p>
                </div>
              </div>
            </div>
          </div>
