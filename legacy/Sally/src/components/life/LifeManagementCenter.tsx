                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-500">{role.name}</h3>
                        <p className="text-sm text-gray-400">{role.tasks} active tasks</p>
                      </div>
                    </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-xl p-6 border border-peacock-200">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-gray-500">{getRoleById(selectedRole)?.name}</h3>
                  <p className="text-sm text-gray-400">Managing {getRoleById(selectedRole)?.tasks} responsibilities</p>
                </div>
              </div>
              ...
              {scheduleItems.map((item, index) => {
                ...
                return (
                  <div key={index} className="flex items-center space-x-4 p-3 hover:bg-gray-50 rounded-lg">
                    <div className="w-20 text-sm font-medium text-gray-400">{item.time}</div>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${role?.color}`}>
                      {role?.icon}
                    </div>
                    <div className="flex-1">
                      <p className="font-medium text-gray-500">{item.task}</p>
                  ...
                  .map(role => (
                    <div key={role.id} className="p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span className="font-medium text-gray-500">{role.name}</span>
                      </div>
