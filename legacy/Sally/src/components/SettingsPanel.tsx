...
export function SettingsPanel() {
  ...
  const { isDesktop, version, isConnected, getSetting, setSetting } = useNativeBridge();
  ...
  // Desktop settings - Now persisted via native bridge API
  ...
  // Load desktop settings from native storage on mount
  useEffect(() => {
    if (!isDesktop || !getSetting) return;

    const loadDesktopSettings = async () => {
      try {
        const savedSettings = await getSetting<DesktopSettings>(DESKTOP_SETTINGS_KEY);
        if (savedSettings) {
          setDesktopSettings(savedSettings);
        }
      } catch (err) {
        console.error('[SettingsPanel] Failed to load desktop settings:', err);
      }
    };

    loadDesktopSettings();
  }, [isDesktop, getSetting]);

  // Save desktop settings to native storage when changed
  const handleDesktopSettingChange = async (key: keyof DesktopSettings, value: boolean) => {
    const newSettings = { ...desktopSettings, [key]: value };
    setDesktopSettings(newSettings);

    if (isDesktop && setSetting) {
      try {
        await setSetting(DESKTOP_SETTINGS_KEY, newSettings);
      } catch (err) {
        console.error('[SettingsPanel] Failed to save desktop settings:', err);
      }
    }
  };
            ...
            <div className="space-y-4">
              <label className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => handleDesktopSettingChange('enableSystemTray', e.target.checked)}
                  className="w-5 h-5 rounded border-gray-600 bg-gray-700 text-violet-600 focus:ring-violet-500"
                />
              ...
              <label className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => handleDesktopSettingChange('minimizeToTray', e.target.checked)}
                  className="w-5 h-5 rounded border-gray-600 bg-gray-700 text-violet-600 focus:ring-violet-500"
              ...
              <label className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => handleDesktopSettingChange('startOnLogin', e.target.checked)}
                  className="w-5 h-5 rounded border-gray-600 bg-gray-700 text-violet-600 focus:ring-violet-500"
                />
              ...
              <label className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => handleDesktopSettingChange('enableNativeNotifications', e.target.checked)}
                  className="w-5 h-5 rounded border-gray-600 bg-gray-700 text-violet-600 focus:ring-violet-500"
                />
