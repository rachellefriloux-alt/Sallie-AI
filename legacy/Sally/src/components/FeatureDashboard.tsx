...
export function FeatureDashboard() {
  ...
  const toggleVoiceInterface = async () => {
    try {
      const response = await fetch(`${API_BASE}/voice/toggle`, {
        method: 'POST'
      });
  ...
  const toggleCloudSync = async () => {
    try {
      const response = await fetch(`${API_BASE}/sync/toggle`, {
        method: 'POST'
      });
  ...
  const handleFeatureClick = async (feature: Feature) => {
    try {
      // Track feature usage
      await fetch(`${API_BASE}/analytics/feature-used`, {
        method: 'POST',
  ...
  const toggleFeature = async (featureId: string, enabled: boolean) => {
    try {
      const response = await fetch(`${API_BASE}/features/${featureId}/toggle`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
