...
import { useLimbicStore, type LimbicState } from '@/store/useLimbicStore';
import { useWebSocket } from '@/hooks/useWebSocket';

const API_BASE = process.env.NEXT_PUBLIC_API_URL || '/api';
const LIMBIC_ENGINE_URL = process.env.NEXT_PUBLIC_API_URL || '/api';
  ...
  useEffect(() => {
    const initializeLimbicEngine = async () => {
      try {
        ...
        const trustRes = await fetch('/api/limbic/trust');
        if (trustRes.ok) {
          const trustData = await trustRes.json();
          setTrustTier(trustData.tier || trustData.current || null);
        }

        const historyRes = await fetch('/api/limbic/history');
        if (historyRes.ok) {
          const historyData = await historyRes.json();
          const formattedHistory = (historyData.history || []).map((entry: any) => ({
            timestamp: entry.timestamp || Date.now(),
            trust: entry.trust || 0,
            warmth: entry.warmth || 0,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            {limbicState ? (
              <LimbicGauges limbicState={limbicState} />
            ) : (
