...
const ghostUrl = process.env.NEXT_PUBLIC_API_URL
  ? `${process.env.NEXT_PUBLIC_API_URL}/api/ghost`
  : '/api/ghost';
...
export function HypothesisManager() {
  ...
  const loadHypotheses = async () => {
    try {
      const response = await fetch(`${ghostUrl}/veto_pending`);
      if (response.ok) {
  ...
  const handleAction = async (hypothesisId: string, action: 'confirm' | 'deny' | 'add_context', context?: string) => {
    try {
      const response = await fetch(`${ghostUrl}/veto_pending/${hypothesisId}`, {
        method: 'PATCH',
          ...
          {pendingHypotheses.map((hypothesis) => (
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => {
                    const context = window.prompt('Add context for this hypothesis:');
                    if (context !== null) handleAction(hypothesis.id, 'add_context', context);
                  }}
