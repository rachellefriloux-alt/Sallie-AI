...
type VoiceCommand = { command: string; action: () => void; description: string };
type VoiceResponse = { text: string; emotion: string; duration: number; audioUrl?: string };
type VoiceControlsProps = { onTranscript: (transcript: string) => void; onVoiceCommand: (command: string) => void; className?: string };
type SpeechRecognitionResult = { transcript: string; confidence: number; isFinal: boolean };
type TextToSpeechRequest = { text: string; emotion: string; voice?: string; speed?: number; pitch?: number };
type TextToSpeechResponse = { audioUrl: string; duration: number; success: boolean };
type VoiceSettings = { language: string; voice: string; speed: number; pitch: number; volume: number };
type SupportedLanguage = 'en-US' | 'en-GB' | 'es-ES' | 'fr-FR';
type VoiceEmotion = 'neutral' | 'happy' | 'sad' | 'excited' | 'calm';
