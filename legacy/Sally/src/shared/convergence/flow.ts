...
import { ENHANCED_GENESIS_QUESTIONS, EnhancedGenesisQuestion } from '../genesis/enhanced_questions';
...
export class ConvergenceFlow {
  ...
  private loadQuestions(): ConvergenceQuestion[] {
    // Load all 29 questions from ENHANCED_GENESIS_QUESTIONS
    return ENHANCED_GENESIS_QUESTIONS.map((q: EnhancedGenesisQuestion): ConvergenceQuestion => ({
      id: q.id,
      phase: q.phase,
      text: q.text,
      purpose: q.purpose,
      extraction_key: q.extraction_key,
      phase_name: q.phase_name,
      phase_description: q.phase_description,
    }));
  }
