...
export class MemStorage implements IStorage {
  ...
  async createPersonaState(personaState: InsertPersonaState): Promise<PersonaState> {
    ...
    this.personaStates.set(newPersonaState.id, newPersonaState);
    return newPersonaState;
  }

  async updatePersonaState(userId: string, updates: Partial<PersonaState>): Promise<PersonaState | undefined> {
    const existingState = Array.from(this.personaStates.values()).find(state => state.userId === userId);
    if (!existingState) return undefined;

    const updatedState = { ...existingState, ...updates, updatedAt: new Date() };
    this.personaStates.set(existingState.id, updatedState);
    return updatedState;
  }

  async deletePersonaState(userId: string): Promise<boolean> {
    const stateToDelete = Array.from(this.personaStates.values()).find(state => state.userId === userId);
    if (!stateToDelete) return false;
    return this.personaStates.delete(stateToDelete.id);
  }
}
